...
#this version includes R2* and SMWI-maps.
...
rm -f files.csv
for input in $(cat ${TEMPLATE_LIST}); do
  echo ${TEMPLATE_DIR}/bet/${input}_t1_brain.nii.gz,${TEMPLATE_DIR}/bet/${input}_smwi_brain.nii.gz >> files.csv
done

#depending on which option it runs a different version on the cluster
if  [ $QSUB_OPTION -eq 4 ]; then
	${CODE_PATH}/antsMultivariateTemplateConstruction2.sh -d 3 -k 2 -w 1x1 -n 0 -f 8x4x2x1 -s 3x2x1x0 -q 100x70x50x10 -n 0 -l 1 -m CC -c 4 -y 0 -o ${TEMPLATE_DIR}/template/multivariate_ -z ${t1_affine_template} -z ${smwi_affine_template} files.csv
else
	antsMultivariateTemplateConstruction2.sh -d 3 -k 2 -w 1x1 -n 0 -f 8x4x2x1 -s 3x2x1x0 -q 100x70x50x10 -n 0 -l 1 -m CC -c 2 -j ${NUM_CORES} -y 0 -o multivariate_ -z ${t1_affine_template} -z ${smwi_affine_template} files.csv
fi
...
`ln -sf multivariate_template1.nii.gz  ants_multivariate_smwi_template.nii.gz`
