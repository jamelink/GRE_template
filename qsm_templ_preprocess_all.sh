...
for input in $(cat ${TEMPLATE_LIST}); do
  ...
  input_r2s="${TEMPLATE_DIR}/template/${input}_r2s_aff"
  if [[ $(imtest ${input_qsm}) -eq "0" ]] || [[ $(imtest ${input_t1}) -eq "0" ]] || [[ $(imtest ${input_smwi}) -eq "0" ]] || [[ $(imtest ${input_r2s}) -eq "0" ]]; then
    echo "${CODE_PATH}/qsm_templ_preprocess_sub_all.sh $TEMPLATE_DIR ${input}" | qsub -l 'walltime=02:00:00,mem=4Gb' -N "${input}_qsm_template_preprocessing"
  else
