...
#this version includes SMWI-maps.
...
#Brain extract the SMWI maps also
echo "Applying mask to SMWI images..."
mask="reoriented/${input}_mask"
input_smwi="reoriented/${input}_smwi"
output_brain="bet/${input}_smwi_brain_uncorrected"
if [ $(imtest ${output_brain}) -eq "0" ]; then
  fslmaths ${input_smwi} -mas ${mask} ${output_brain}
fi

#Normalization step
echo "Thresholding SMWI images..."
input_smwi="bet/${input}_smwi_brain_uncorrected"
output_brain="bet/${input}_smwi_brain"
if [ $(imtest ${output_brain}) -eq "0" ]; then
  fslmaths ${input_smwi} -uthr 1000 ${output_brain}
fi
...
echo "Registering SMWI to T1..."
input_smwi="bet/${input}_smwi_brain"
output_smwi_reg="qsm_to_t1_reg/${input}_smwi"
if [ $(imtest ${output_smwi_reg}) -eq "0" ]; then
  flirt -in ${input_smwi} -ref bet/${input}_t1_brain -dof 6 -cost normmi -searchcost normmi -interp spline -out ${output_smwi_reg} -omat ${output_smwi_reg}.mat
fi
...
output_smwi="${input}_smwi_aff"
if [ $(imtest ${output_smwi}) -eq "0" ]; then
  input_smwi="../bet/${input}_smwi_brain"
  convert_xfm -omat ${input}_smwi_aff.mat -concat ${input}_t1_aff.mat ../qsm_to_t1_reg/${input}_smwi.mat
  flirt -in ${input_smwi} -ref ${TEMPLATE_TARGET} -init ${input}_smwi_aff.mat -applyxfm -out ${output_smwi} -interp sinc
fi
