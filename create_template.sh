...
rm -f files.csv
for input in $(cat ${TEMPLATE_LIST}); do
  echo ${TEMPLATE_DIR}/bet/${input}_t1_brain.nii.gz,${TEMPLATE_DIR}/qsm_to_t1_reg/${input}_qsm_offset.nii.gz >> files.csv
done

#depending on which option it runs a different version on the cluster
if  [ $QSUB_OPTION -eq 4 ]; then
	${CODE_PATH}/antsMultivariateTemplateConstruction2.sh -d 3 -k 2 -w 1x1 -n 0 -f 8x4x2x1 -s 3x2x1x0 -q 100x70x50x10 -n 0 -l 1 -m CC -c 4 -y 0 -o ${TEMPLATE_DIR}/template/multivariate_ -z ${t1_affine_template} -z ${qsm_affine_template} files.csv
else
